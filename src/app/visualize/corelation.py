...
def visualize_correlation(filePath: str):
    ...
    # =============================================================================
    data_dir = "data"
    file_name = filePath if (filePath=="" or None) else "dataset_1/dataset_1.csv"
    ...
    if len(top_categories) >= 2:
        ...
        combined_symptoms = list(set(cat1_data.index.tolist() + cat2_data.index.tolist())) # type: ignore
        
        # Recalculate difference using the top symptoms across both
        cat1_freq = df[df[TARGET_COL] == cat1][combined_symptoms].sum() # type: ignore
        cat2_freq = df[df[TARGET_COL] == cat2][combined_symptoms].sum() # type: ignore
        
        diff = cat1_freq.sub(cat2_freq, fill_value=0).sort_values(key=abs, ascending=False)
        
        print(f"\n--- Key Symptoms Unique to {cat1} (Positive numbers) vs. {cat2} (Negative numbers) ---") # type: ignore
